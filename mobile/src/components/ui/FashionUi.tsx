...
export function GradientScreenHeader({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle: string }) {
  const { rounded, colors } = useTheme();
  return <Animated.View entering={FadeInDown.duration(420)}><View style={[styles.hero, { borderRadius: rounded.lg, backgroundColor: colors.primary, shadowColor: colors.shadow }]}>
    <View style={styles.heroIcon}><Ionicons name="sparkles" size={18} color={colors.primary} /></View>
    <AppText variant="caption" color={colors.sand}>{eyebrow}</AppText>
...
export function FloatingActionButton({ onPress }: { onPress: () => void }) {
  ...
  return <Animated.View style={[styles.fabWrap, style]}><Pressable onPress={onPress} style={[styles.fab, { borderRadius: rounded.lg, backgroundColor: colors.primary, shadowColor: colors.shadow }]}><Ionicons name="add" size={22} color={colors.textInverse} /><AppText variant="label" color={colors.textInverse}>Thêm mới</AppText></Pressable></Animated.View>;
}

export function SearchBar({ value, onChangeText }: { value: string; onChangeText: (value: string) => void }) {
  const { colors, rounded } = useTheme();
  return <View style={[styles.search, { backgroundColor: colors.surface, borderColor: colors.sand, borderRadius: rounded.full }]}>
...
export function FilterPill({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  const { colors, rounded } = useTheme();
  return <Pressable onPress={onPress} style={[styles.pill, { borderRadius: rounded.full, backgroundColor: active ? colors.primary : colors.surface, borderColor: active ? colors.primary : colors.sand }]}>
...
export function StatBubble({ icon, value, label, tint, delay = 0 }: { icon: keyof typeof Ionicons.glyphMap; value: number; label: string; tint: string; delay?: number }) {
  const { rounded, colors } = useTheme();
  return <Animated.View entering={FadeInDown.delay(delay).duration(350)} style={[styles.stat, { backgroundColor: tint, borderRadius: rounded.lg, shadowColor: colors.shadow }]}>
    <Ionicons name={icon} size={18} color={colors.primary} /><AppText variant="h2">{value}</AppText><AppText variant="caption" muted>{label}</AppText>
  </Animated.View>;
}

export function VibeBadge({ label, color }: { label: string; color?: string }) {
  const { colors, rounded } = useTheme();
  return <View style={[styles.badge, { backgroundColor: color ?? colors.sand, borderRadius: rounded.full }]}><AppText variant="caption">{label}</AppText></View>;
}
...
export function ClosetItemCard({ item, onPress, onFavorite }: { item: ClothingItem; onPress: () => void; onFavorite: () => void }) {
  ...
  return <AnimatedPressable onPress={onPress} style={styles.cardWrap}><View style={[styles.card, { borderRadius: rounded.lg, backgroundColor: colors.surface, shadowColor: colors.shadow }]}>
    {imageFailed ? <View style={[styles.cardImage, styles.imageFallback]}><Ionicons name="shirt-outline" size={42} color={colors.primary} /><AppText variant="caption" color={colors.primary}>ẢNH MÓN ĐỒ</AppText></View> : <Image source={{ uri: item.imageUrl }} style={styles.cardImage} contentFit="cover" onError={() => setImageFailed(true)} />}
    <View style={[StyleSheet.absoluteFill, styles.cardOverlay]} />
    <View style={styles.cardTop}><VibeBadge label={item.type} color="rgba(255,255,255,0.84)" /><Pressable onPress={onFavorite} hitSlop={12}><Ionicons name={item.isFavorite ? 'heart' : 'heart-outline'} size={22} color={item.isFavorite ? colors.primary : '#fff'} /></Pressable></View>
...
export function EmptyClosetState({ onPress }: { onPress: () => void }) {
  const { colors, rounded } = useTheme();
  return <View style={[styles.empty, { borderRadius: rounded.lg, backgroundColor: colors.primary }]}>
...
const styles = StyleSheet.create({
  hero: { padding: 22, overflow: 'hidden', minHeight: 210, justifyContent: 'flex-end', shadowOpacity: 0.08, shadowRadius: 2, shadowOffset: { width: 0, height: 1 } },
  ...
  fabWrap: { position: 'absolute', right: 18, bottom: 108, zIndex: 20 }, fab: { minHeight: 54, paddingHorizontal: 17, flexDirection: 'row', gap: 7, alignItems: 'center', justifyContent: 'center', shadowOpacity: 0.1, shadowRadius: 2, shadowOffset: { width: 0, height: 1 } },
  ...
  pill: { paddingHorizontal: 13, paddingVertical: 9, borderWidth: 1 }, stat: { flex: 1, minWidth: '46%', padding: 12, gap: 2, shadowOpacity: 0.08, shadowRadius: 2, shadowOffset: { width: 0, height: 1 } },
