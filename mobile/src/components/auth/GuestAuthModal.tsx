...
export function GuestAuthModal() {
  const router = useRouter(); const { colors, radius } = useTheme();
  const visible = useAuthStore((s) => s.showGuestPrompt); const close = useAuthStore((s) => s.closeGuestPrompt);
  return <Modal visible={visible} transparent animationType="fade"><View style={styles.overlay}><View style={[styles.card, { backgroundColor: colors.surface, borderRadius: radius.xl }]}>
    ...
    <Button label="Đăng ký miễn phí" onPress={() => { close(); router.push('/auth/register'); }} />
