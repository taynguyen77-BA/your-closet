...
const challenges = [
  ['7-Day Outfit Remix', 'Biến 7 món quen thành 7 look mới', '5/7 ngày', '680', '120 XP', '#2F6F73'],
  ['No-buy Week', 'Mặc lại tủ đồ, không mua mới trong tuần', '3/7 ngày', '1.2K', '80 XP', '#2E2347'],
  ['Office Glow-up', 'Nâng vibe công sở bằng item sẵn có', '2/5 look', '426', '70 XP', '#8F395A'],
  ['Summer Color Pop', 'Thêm một màu nổi bật vào outfit', '1/3 look', '905', '60 XP', '#151310'],
];
...
export default function CommunityHubScreen() {
  ...
  const { isAuthenticated, isGuest, requireAccount } = useAuthStore();
  ...
  const visibleTabs = isPublicViewer ? tabs.filter((tab) => tab.id !== 'mine' && tab.id !== 'challenges') : tabs;
  ...
  const listingCard = (item: CommunityListing, showStatus = false) => (
    <Pressable key={item.id} onPress={() => router.push(`/community/${item.id}`)} style={[styles.product, { backgroundColor: colors.surface, borderRadius: radius.xl }]}>
      ...
      {!isPublicViewer ? <Pressable accessibilityLabel={`Lưu ${item.title}`} onPress={() => setSavedListingIds((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id])} style={[styles.heart, { backgroundColor: colors.surfaceGlass }]}><Ionicons name={savedListingIds.includes(item.id) ? 'heart' : 'heart-outline'} size={17} color={colors.community} /></Pressable> : null}
  ...
  return (
    <Screen padded={false} scroll={false}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={gradients.community} style={[styles.hero, { borderBottomLeftRadius: radius.xxl, borderBottomRightRadius: radius.xxl, shadowColor: colors.shadow }]}>
          <View style={styles.heroIcon}><Ionicons name="people-outline" size={19} color={colors.community} /></View>
          <AppText variant="caption" color={colors.accentLight}>CỘNG ĐỒNG PHONG CÁCH</AppText>
          <AppText variant="display" color={colors.textInverse}>Vòng tròn phong cách</AppText>
          <AppText variant="body" color={colors.warmLight} style={styles.heroCopy}>Mix đồ, pass đồ, săn outfit cùng cộng đồng</AppText>
          <View style={styles.heroActions}>
            <Button label={isPublicViewer ? 'Đăng nhập để đăng' : 'Đăng outfit'} small icon="camera-outline" variant="secondary" onPress={() => requireSignedIn(() => router.push('/community/create?intent=post'))} style={{ marginRight: 8 }} />
            <Button label="Pass đồ ngay" small icon="pricetag-outline" variant="secondary" onPress={() => requireSignedIn(() => router.push('/community/create'))} />
          </View>
        </LinearGradient>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabsWrap} contentContainerStyle={[styles.tabs, { paddingHorizontal: spacing.lg }]}>
          {visibleTabs.map((tab) => <Pressable key={tab.id} onPress={() => setActive(tab.id)} style={[styles.tab, { backgroundColor: active === tab.id ? colors.deepPurple : colors.surface, borderColor: colors.border }]}><AppText variant="label" color={active === tab.id ? colors.textInverse : colors.text}>{tab.label}</AppText></Pressable>)}
        </ScrollView>

        <View style={{ paddingHorizontal: spacing.lg }}>
        {active === 'feed' && <>
          {trends.map((trend, index) => <View key={trend.id} style={[styles.post, { backgroundColor: colors.surface, borderRadius: radius.xl }]}>
            <View style={styles.author}><SafeImage source={{ uri: index ? approved[0]?.sellerAvatarUrl ?? user.avatarUrl : user.avatarUrl }} style={styles.avatar} /><View style={{ flex: 1 }}><AppText variant="label">{index || isPublicViewer ? 'Phong cách cộng đồng' : user.username}</AppText><AppText variant="bodySmall" muted>{trend.season} · {isPublicViewer ? 'Xu hướng nổi bật' : 'Phong cách của bạn'}</AppText></View><Ionicons name="ellipsis-horizontal" size={20} color={colors.textMuted} /></View>
            ...
            <Button label={isPublicViewer ? 'Đăng nhập để thử style' : 'Thử style này'} variant="ai" small icon="sparkles" onPress={() => requireSignedIn(() => router.push('/(tabs)/try-on'))} style={{ marginTop: 12, alignSelf: 'flex-start' }} />
          </View>)}
        </>}
        {active === 'challenges' && <>
          ...
          {challenges.map(([title, description, progress, people, reward, color]) => <View key={title} style={[styles.challenge, { backgroundColor: color, borderRadius: radius.xl }]}><AppText variant="caption" color={colors.textInverse}>{people} NGƯỜI THAM GIA · {reward}</AppText><AppText variant="h2" color={colors.textInverse}>{title}</AppText><AppText variant="bodySmall" color={colors.textInverse}>{description}</AppText><AppText variant="label" color={colors.textInverse} style={{ marginTop: 12 }}>{progress}</AppText><Button label="Tham gia ngay" small variant="secondary" onPress={() => requireSignedIn(() => router.push(title === 'No-buy Week' ? '/missions' : '/community/create?intent=post'))} style={{ alignSelf: 'flex-start', marginTop: 10 }} /></View>)}
        </>}
        {active === 'exchange' && <>
          <View style={styles.sectionTop}><View><AppText variant="h1">Góc trao đổi</AppText><AppText variant="bodySmall" muted>Đổi món cũ, tìm một phong cách mới.</AppText></View><Button label="Đăng món" small variant="community" icon="add" onPress={() => requireSignedIn(() => router.push('/community/create?type=trade'))} /></View>
      ...
      {!isPublicViewer ? <FloatingActionButton onPress={() => router.push('/community/create')} /> : null}
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  hero: { minHeight: 250, paddingHorizontal: 22, paddingTop: 26, paddingBottom: 24, justifyContent: 'flex-end', shadowOpacity: 0.18, shadowRadius: 20, shadowOffset: { width: 0, height: 12 }, elevation: 5 },
  heroIcon: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,249,241,0.92)', alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
  heroCopy: { maxWidth: 330, lineHeight: 22 },
  ...
  post: { padding: 10, marginBottom: 16, shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: 9 }, elevation: 3 },
  ...
  challenge: { minHeight: 164, padding: 18, marginBottom: 12, justifyContent: 'center' },
  ...
  product: { width: '48.5%', overflow: 'hidden', marginBottom: 12, shadowOpacity: 0.08, shadowRadius: 15, shadowOffset: { width: 0, height: 8 }, elevation: 3 },
