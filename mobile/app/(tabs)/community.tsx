...
const tabs: { id: HubTab; label: string }[] = [
  { id: 'feed', label: 'Feed' }, { id: 'challenges', label: 'Challenges' },
  { id: 'exchange', label: 'Exchange' }, { id: 'marketplace', label: 'Marketplace' }, { id: 'mine', label: 'Mine' },
];
const challenges = [
  ['7-Day Outfit Remix', 'Biến 7 món quen thành 7 look mới', '5/7 ngày', '680', '120 XP', '#FF3D9A'],
  ['No-buy Week', 'Mặc lại tủ đồ, không mua mới trong tuần', '3/7 ngày', '1.2K', '80 XP', '#16A374'],
  ['Office Glow-up', 'Nâng vibe công sở bằng item sẵn có', '2/5 look', '426', '70 XP', '#7C3AED'],
  ['Summer Color Pop', 'Thêm một màu nổi bật vào outfit', '1/3 look', '905', '60 XP', '#F59E0B'],
];
...
export default function CommunityHubScreen() {
  ...
  const listingCard = (item: CommunityListing, showStatus = false) => (
    <Pressable key={item.id} onPress={() => router.push(`/community/${item.id}`)} style={[styles.product, { backgroundColor: colors.surface, borderRadius: radius.xl }]}>
      <Image source={{ uri: item.imageUrls[0] }} style={styles.productImage} contentFit="cover" />
      ...
      <View style={styles.productInfo}>
        ...
        <AppText variant="h3" color={item.price ? colors.community : colors.marketplace}>{item.price ? `${item.price.toLocaleString('vi-VN')}đ` : item.listingType === 'trade' ? 'Open to trade' : 'Free'}</AppText>
        <AppText variant="bodySmall" muted>{item.size ?? 'Freesize'} · {item.location}</AppText>
        <View style={styles.seller}><Image source={{ uri: item.sellerAvatarUrl ?? user.avatarUrl }} style={styles.miniAvatar} /><AppText variant="caption" muted>{item.sellerName}</AppText></View>
  ...
  return (
    <Screen padded={false}>
      <LinearGradient colors={gradients.community} style={[styles.hero, { borderBottomLeftRadius: radius.xxl, borderBottomRightRadius: radius.xxl }]}>
        <AppText variant="caption" color={colors.textInverse}>YOUR STYLE PEOPLE</AppText>
        <AppText variant="display" color={colors.textInverse}>Style Circle</AppText>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.tabs, { paddingHorizontal: spacing.lg }]}>
      ...
      <View style={{ paddingHorizontal: spacing.lg }}>
        {active === 'feed' && <>
          {trends.map((trend, index) => <View key={trend.id} style={[styles.post, { backgroundColor: colors.surface, borderRadius: radius.xl }]}>
            <View style={styles.author}><Image source={{ uri: index ? approved[0]?.sellerAvatarUrl ?? user.avatarUrl : user.avatarUrl }} style={styles.avatar} /><View style={{ flex: 1 }}><AppText variant="label">{index ? 'stylebylinh' : user.username}</AppText><AppText variant="bodySmall" muted>{trend.season} · Your vibe</AppText></View><Ionicons name="ellipsis-horizontal" size={20} color={colors.textMuted} /></View>
            <Image source={{ uri: trend.previewImageUrl }} style={styles.look} contentFit="cover" />
        ...
        {active === 'challenges' && <>
          <AppText variant="h1">Glow-up challenges</AppText><AppText variant="bodySmall" muted style={{ marginBottom: 12 }}>Chơi cùng cộng đồng, mở thêm XP và lượt AI.</AppText>
        ...
        {active === 'exchange' && <>
          <View style={styles.sectionTop}><View><AppText variant="h1">Swap corner</AppText><AppText variant="bodySmall" muted>Đổi món cũ, tìm một vibe mới.</AppText></View><Button label="Đăng món" small variant="community" icon="add" onPress={() => router.push('/community/create?type=trade')} /></View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginVertical: 14 }}>{['Gần bạn', 'Freesize', 'Like new', 'Free'].map((label) => <Pressable key={label} onPress={() => setExchangeFilter(label)} style={[styles.filter, { backgroundColor: exchangeFilter === label ? colors.mint : colors.surface, borderColor: colors.border }]}><AppText variant="bodySmall">{label}</AppText></Pressable>)}</ScrollView>
          <View style={styles.grid}>{exchange.map((item) => listingCard(item))}</View>
        </>}
        {active === 'marketplace' && <>
          <View style={styles.sectionTop}><View><AppText variant="h1">Fresh finds</AppText><AppText variant="bodySmall" muted>Pre-loved pieces, main character energy.</AppText></View><Button label="Xem tất cả" small variant="secondary" onPress={() => router.push('/community')} /></View>
          <View style={styles.grid}>{marketplace.map((item) => listingCard(item))}</View>
        </>}
        {active === 'mine' && <>
          <View style={styles.sectionTop}><View><AppText variant="h1">My listings</AppText><AppText variant="bodySmall" muted>Theo dõi trạng thái các món bạn đã đăng.</AppText></View><Button label="Đăng mới" small variant="community" icon="add" onPress={() => router.push('/community/create')} /></View>
