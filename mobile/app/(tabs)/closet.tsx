...
export default function ClosetScreen() {
  ...
  const header = <View style={{ gap: spacing.md }}>
    ...
    <View style={[styles.health, { borderRadius: rounded.lg, backgroundColor: colors.primary }]}>
      <View style={styles.healthTop}><View style={{ flex: 1 }}><AppText variant="caption" color={colors.sand}>SỨC KHỎE TỦ ĐỒ</AppText><AppText variant="h1" color={colors.textInverse}>{wardrobeScore}/100 · Đang rất ổn</AppText></View><AppText variant="h2" color={colors.sand}>{wardrobeScore}</AppText></View>
  ...
  return <View style={{ flex: 1, backgroundColor: colors.background }}>
    ...
    <Modal visible={bulkDrafts.length > 0} animationType="slide" transparent>
      <View style={styles.overlay}>
        <View style={[styles.reviewModal, { backgroundColor: colors.surface, borderRadius: rounded.lg }]}>
          {currentBulkDraft ? (
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 12 }}>
              ...
              <SafeImage source={{ uri: currentBulkDraft.originalImageUrl }} style={[styles.reviewImage, { borderRadius: rounded.DEFAULT, width: '100%' }]} contentFit="cover" />
