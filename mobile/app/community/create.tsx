...
export default function CreateListingScreen() {
  ...
  return <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={{ padding: spacing.lg }}>
    ...
    {step === 1 && <><AppText variant="h2">Chọn món từ closet</AppText><AppText variant="bodySmall" muted style={styles.help}>Ảnh đẹp sẵn rồi, bạn chỉ cần chọn món muốn pass.</AppText><View style={styles.grid}>{clothing.map((value) => <Pressable key={value.id} onPress={() => setSelectedId(value.id)} style={[styles.item, { borderColor: selectedId === value.id ? colors.community : colors.border, borderRadius: radius.lg }]}><Image source={{ uri: value.imageUrl }} style={styles.itemImage} /><AppText variant="label" numberOfLines={1} style={{ padding: 8 }}>{value.name}</AppText></Pressable>)}</View></>}
    ...
    {step === 4 && item && <><AppText variant="h2">Preview listing</AppText><View style={[styles.preview, { backgroundColor: colors.surface, borderRadius: radius.xl }]}><Image source={{ uri: item.imageUrl }} style={styles.previewImage} /><View style={{ padding: 14 }}><AppText variant="h2">{title}</AppText><AppText variant="h1" color={colors.community}>{listingType === 'sale' ? `${Number(price).toLocaleString('vi-VN')}đ` : listingType === 'trade' ? 'Open to trade' : 'Free'}</AppText><AppText variant="bodySmall" muted>{size} · {condition} · {location}</AppText><AppText style={{ marginTop: 10 }}>{description}</AppText></View></View></>}
    {step === 5 && <LinearGradient colors={gradients.marketplace} style={[styles.done, { borderRadius: radius.xl }]}><AppText variant="display" color={colors.textInverse}>Đã gửi duyệt!</AppText><AppText color={colors.textInverse}>Món của bạn đã nằm trong My Listings với trạng thái Chờ duyệt.</AppText><Button label="Xem My Listings" variant="secondary" onPress={() => router.replace('/community?filter=mine')} style={{ marginTop: 16 }} /><Button label="Về Style Circle" variant="ghost" onPress={() => router.replace('/(tabs)/community' as never)} style={{ marginTop: 8 }} /></LinearGradient>}
