...
export default function Page() {
 ...
 return <div><PageHeader title="Dashboard" description="Firestore real-time overview"/><ApiStatusCard/><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(([label,value,icon])=><KpiCard key={label} label={label} value={value} icon={icon}/>)}</div></div>;
}
